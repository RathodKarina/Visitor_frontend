    ...
    <div>
      ...
      <div className="page-container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap:'wrap', gap:'12px' }}>
          <h2 className="dashboard-title" style={{ marginBottom: 0 }}>🛡️ Security: Manage Visits</h2>
        </div>
        ...
        <div className="table-card glass-panel">
          <table className="data-table">
            <thead>
              <tr>{['Visitor','Host','Dept','Purpose','Date','Status','Actions'].map(h => <th key={h}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ?
                <tr><td colSpan="7" style={{ textAlign:'center', color:'#94a3b8', padding:'40px' }}>No records found.</td></tr> :
                filtered.map(v => (
                  <tr key={v._id}>
                    <td>
